...
class RequestsTransport(object):
    ...
    def execute(self, query, params=None):
        """Query is returning rows and these rows should be parsed or there
        ...
        for line in lines:
            yield [(converter(x) if converter else x)
    ...
    def _send(self, data, params=None, stream=False):
        ...
        r = requests.post(self.db_url, auth=self.auth, params=params,
                          data=data, stream=stream, timeout=self.timeout)
