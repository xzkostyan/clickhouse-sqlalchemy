...
class DDLTestCase(BaseTestCase):
    ...
    def test_create_table_nested_types(self):
        ...
        self.assertEqual(
            self.compile(CreateTable(table)),
            "CREATE TABLE t1 (x Int32, y Array(String)) ENGINE = Memory",
        )
    ...
    def test_create_table_nullable_nested_nullable(self):
        table = Table(
            ...
            Column("x", types.Int32, primary_key=True),
            Column(
                "y", types.Nullable(types.Array(types.Nullable(types.String)))
            ),
            engines.Memory(),
        )
    ...
    def test_add_column(self):
        ...
        self.assertEqual(self.compile(CreateColumn(col)), "x2 Int8 AFTER x1")
    ...
    def test_create_table_tuple(self):
        ...
        self.assertEqual(
            self.compile(CreateTable(table)),
            "CREATE TABLE t1 (x Tuple(Int8, Float32)) ENGINE = Memory",
        )
    ...
    def test_create_table_named_tuple(self):
        table = Table(
            ...
            Column(
                "x",
                types.Tuple(("name", types.String), ("value", types.Float32)),
            ),
            engines.Memory(),
        )
    ...
    def test_create_table_map(self):
        ...
        self.assertEqual(
            self.compile(CreateTable(table)),
            "CREATE TABLE t1 (x Map(String, String)) ENGINE = Memory",
        )
    ...
    def test_create_simple_aggregate_function(self):
        table = Table(
            ...
            Column(
                "total",
                types.SimpleAggregateFunction(func.sum(), types.UInt32),
            ),
            engines.Memory(),
        )
    ...
    def test_table_create_on_cluster(self):
        create_sql = (
            "CREATE TABLE t1 ON CLUSTER test_cluster (x Int32) ENGINE = Memory"
        )
    ...
    def test_create_drop_mat_view(self):
        ...
        MatView = MaterializedView(
            ...
            select(
                ...
                func.sum(Statistics.metric1 * Statistics.sign).label(
                    "metric1"
                ),
            )
            .where(Statistics.grouping > 42)
            .group_by(Statistics.date),
        )
