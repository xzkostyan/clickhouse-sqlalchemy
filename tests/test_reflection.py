...
class ReflectionTestCase(BaseTestCase):
    def test_server_default(self):
        ...
        with self.create_table(table):
            self.assertEqual(
                [
                    c["default"]
                    for c in inspect(self.session.bind).get_columns("t")
                ],
                [None, "''"],
            )
    ...
    def _type_round_trip(self, *types):
        ...
        args = [Column("t%d" % i, type_) for i, type_ in enumerate(types)] + [
            engines.Memory()
        ]
    ...
    def test_array_of_array(self):
        coltype = self._type_round_trip(types.Array(types.Array(types.Int32)))[
            0
        ]["type"]
    ...
    def test_map(self):
        coltype = self._type_round_trip(types.Map(types.String, types.String))[
            0
        ]["type"]
    ...
    def test_datetime64(self):
        coltype = self._type_round_trip(types.DateTime64(2, "Europe/Moscow"))[
            0
        ]["type"]
        ...
        coltype = self._type_round_trip(types.DateTime64())[0]["type"]
    ...
    def test_datetime(self):
        coltype = self._type_round_trip(types.DateTime("Europe/Moscow"))[0][
            "type"
        ]
        ...
        coltype = self._type_round_trip(types.DateTime())[0]["type"]
    ...
    def test_aggregate_function(self):
        ...
        coltype = self._type_round_trip(
            types.AggregateFunction("sum", types.Decimal(18, 2))
        )[0]["type"]
        self.assertIsInstance(coltype, types.AggregateFunction)
        self.assertEqual(coltype.agg_func, "sum")
        self.assertEqual(len(coltype.nested_types), 1)
    ...
    def test_simple_aggregate_function(self):
        ...
        coltype = self._type_round_trip(
            types.SimpleAggregateFunction("sum", types.Float64)
        )[0]["type"]
        self.assertIsInstance(coltype, types.SimpleAggregateFunction)
        self.assertEqual(coltype.agg_func, "sum")
        self.assertEqual(len(coltype.nested_types), 1)
