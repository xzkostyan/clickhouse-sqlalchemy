...
class VisitTestCase(CompilationTestCase):
    ...
    def test_array(self):
        self.assertEqual(
            self.compile(types.Array(types.Int32())), "Array(Int32)"
        )
        self.assertEqual(
    ...
    def test_enum(self):
        ...
        self.assertEqual(
            self.compile(types.Enum(MyEnum)), "Enum('foo' = 100, 'bar' = 500)"
        )
...
class VisitNativeTestCase(NativeSessionTestCase):
    def test_insert_no_templates_after_value(self):
        ...
        table = Table(
            "t1", self.metadata(), Column("x", types.Int32), engines.Memory()
        )
        self.assertEqual(
            self.compile(table.insert()), "INSERT INTO t1 (x) VALUES"
        )
