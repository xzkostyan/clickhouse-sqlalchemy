...
class SelectTestCase(BaseTestCase):
    ...
    def test_group_by_with_modifiers(self):
        ...
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 GROUP BY t1.x"
        )

        query = select(table.c.x).group_by(table.c.x).with_cube()
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 GROUP BY t1.x WITH CUBE"
        )
        ...
        query = select(table.c.x).group_by(table.c.x).with_cube().with_totals()
        self.assertEqual(
            self.compile(query),
        ...
        query = (
            select(table.c.x).group_by(table.c.x).with_rollup().with_totals()
        )
        self.assertEqual(
            self.compile(query),
    ...
    def test_final(self):
        ...
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 FINAL GROUP BY t1.x"
        )
    ...
    def test_nested_left_array_join(self):
        ...
        query = select(
            table.c.parent.child1,
            table.c.parent.child2,
        ).left_array_join(table.c.parent)
        self.assertEqual(
            self.compile(query),
            "SELECT t1.parent.child1, t1.parent.child2 "
            "FROM t1 "
        ...
        query = select(
            table.c.parent.child1,
            table.c.parent.child2,
        ).left_array_join(parent_labeled)
        self.assertEqual(
            self.compile(query),
            "SELECT t1.parent.child1, t1.parent.child2 "
            "FROM t1 "
    ...
    def test_join(self):
        ...
        def make_statement_select_join(
            type=None,
            strictness=None,
            distribution=None,
            full=False,
        ...
        for make_statement in (make_statement, make_statement_select_join):
            ...
            self.assertEqual(
                self.compile(
                    make_statement(type="INNER", distribution="global")
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        type="INNER", distribution="global", strictness="any"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        type="INNER", distribution="global", strictness="all"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        type="CROSS", distribution="global", strictness="all"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(make_statement(isouter=False, full=False)),
                "SELECT table_1.x FROM table_1 "
                "INNER JOIN table_2 ON table_2.y = table_1.x",
            )

            self.assertEqual(
                self.compile(make_statement(isouter=True, full=False)),
                "SELECT table_1.x FROM table_1 "
                "LEFT OUTER JOIN table_2 ON table_2.y = table_1.x",
            )

            self.assertEqual(
                self.compile(make_statement(isouter=True, full=True)),
                "SELECT table_1.x FROM table_1 "
                "FULL OUTER JOIN table_2 ON table_2.y = table_1.x",
            )

            self.assertEqual(
                self.compile(make_statement(isouter=False, full=True)),
                "SELECT table_1.x FROM table_1 "
                "FULL OUTER JOIN table_2 ON table_2.y = table_1.x",
            )
            ...
            self.assertEqual(
                self.compile(
                    make_statement(isouter=False, full=False, type="INNER")
                ),
                "SELECT table_1.x FROM table_1 "
                "INNER JOIN table_2 ON table_2.y = table_1.x",
            )
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        isouter=False, full=False, type="LEFT OUTER"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
                "LEFT OUTER JOIN table_2 ON table_2.y = table_1.x",
    ...
    def test_distinct_on(self):
        ...
        self.assertEqual(
            self.compile(query), "SELECT DISTINCT ON (t1.x) t1.x FROM t1"
        )
    ...
    def test_nested_map_type(self):
        table = self._make_table(
            ...
            Column(
                "y",
                types.Map(types.String, types.Map(types.String, types.String)),
            ),
        )
