...
class ClickHouseDialectTestCase(BaseTestCase):
    ...
    def test_has_table(self):
        self.assertFalse(self.dialect.has_table(self.session, self.table.name))

        self.table.create(self.session.bind)
        self.assertTrue(self.dialect.has_table(self.session, self.table.name))
    ...
    def test_primary_keys(self):
        ...
        con = self.dialect.get_pk_constraint(
            self.session, self.table_with_primary.name
        )
...
class ClickHouseAsynchDialectTestCase(BaseAsynchTestCase):
    ...
    async def test_empty_set_expr(self):
        ...
        def _run(session):
            return (
                session.query(numbers.c.number)
                .filter(numbers.c.number.in_([]))
                .all()
            )
